...
namespace
{
			...
			const Relations &elementBRelations, RelationType relationType)
	{
		for (const Element &elementFromElementARelations: elementARelations)
		{
			...
			addRelationBetweenElementsRelations(elementBRelations,
	...
	void
	addAndClosureRelationBetweenElements(unsigned long id, char const *value1,
			char const *value2)
	{
...
//        }
	...
	bool noOtherWay(unsigned long id, char const *value1, char const *value2)
	{
		...
		for (const auto &elem : greaterThanE1)
		{
			...
			if (checkIfElementIsLessThanTheOther(id,
					reinterpret_cast<const char *>(&elem), value2))
			{
				return false;
			}
		}
